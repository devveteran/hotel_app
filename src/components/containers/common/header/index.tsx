...
import SignupModal from "@containers/common/signup_modal";
...
const Header = ({scroll, showNavButtons} : PropType) => {
    ...
    return (
        <>
                ...
                <nav className="navbar navbar-expand-xl">
                    <div className="container">
                        ...
                        <Navbar id="home-nav-notification" expand="lg" className='nav flex-row align-items-center list-unstyled ms-xl-auto'>
                            ...
                            {
                                (userInfo.name === '') ? (
                                    <button className="btn btn-md btn-primary ms-3 my-auto rounded-3"
                                        ...
                                        <Navbar.Collapse id="basic-navbar-user" className="nav-item ms-3 dropdown">
                                            <Nav className="me-auto" style={{position:'relative'}}>
                                                ...
                                                <NavDropdownMenu title="" className={`item-user`} renderMenuOnMount={true} id="nav-profile">
                                                    ...
                                                    <NavDropdown.Item href="#action/8.1" className='menu-icon-item bg-danger-soft-hover' id='navmenu-user-signout'
                                                    onClick={signOut}>Sign Out</NavDropdown.Item>
                                                    <hr className="dropdown-divider"/>
            ...
            {
                modalState === 1 && (
                    <SignInModal onCloseModal={closeModal} onOpenSignUp={() => setModalState(2)}/>
                )
            }
            {
                modalState === 2 && (
